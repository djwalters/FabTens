function MILTensorPlot(idx,MIL,MILCi,endtime)
...
%% Tensor Ratio
figure('Name','MIL Tensor','NumberTitle','off')
% Plot value and confidence interval of contact tensor coefficient F11
hE = errorbar(idx,MIL(1,1,:),MIL(1,1,:)-MILCi(1,1,:,1),...
    MILCi(1,1,:,2)-MIL(1,1,:),'rsquare','MarkerSize',msize);
errorbarwidth(hE,ebwidth);

% Plot value and confidence interval of contact tensor coefficient F22
hold on
hE = errorbar(idx,MIL(2,2,:),MIL(2,2,:)-MILCi(2,2,:,1),...
    MILCi(2,2,:,2)-MIL(2,2,:),'bdiamond','MarkerSize',msize+1);
errorbarwidth(hE,ebwidth);

% Plot value and confidence interval of contact tensor coefficient F33
hold on
hE = errorbar(idx,MIL(3,3,:),MIL(3,3,:)-MILCi(3,3,:,1),...
    MILCi(3,3,:,2)-MIL(3,3,:),'go','MarkerSize',msize);
errorbarwidth(hE,ebwidth);

% Adjust format and appearance of contact tensor coefficient plot
grid
axis([-1 (endtime) 0.2 0.5])
y1 = ylabel('MIL Tensor Diagonals (-)');
